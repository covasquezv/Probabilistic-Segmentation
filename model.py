...
def Res_Unet(x):
    ...
    c1, p1 = blocks.res_down(input=x, dim_out=f[0], name="down1") #128 -> 64
    c2, p2 = blocks.res_down(input=p1, dim_out=f[1], name="down2")  #64 -> 32
    c3, p3 = blocks.res_down(input=p2, dim_out=f[2], name="down3")  #32 -> 16
    c4, p4 = blocks.res_down(input=p3, dim_out=f[3], name="down4")  #16->8

    b = blocks.bottleneck(input=p4, dim_out=f[4], name="bottleneck")

    u1 = blocks.res_up(input=b, skip=c4, dim_out=f[3], name="up1") #8 -> 16
    u2 = blocks.res_up(input=u1, skip=c3, dim_out=f[2], name="up2") #16 -> 32
    u3 = blocks.res_up(input=u2, skip=c2, dim_out=f[1], name="up3") #32 -> 64
    u4 = blocks.res_up(input=u3, skip=c1, dim_out=f[0], name="up4") #64 -> 128
...
def encoder(x, n):
    c1, p1 = blocks.res_down(input=x, dim_out=f[0], name="enc1_"+n) #128 -> 64
    c2, p2 = blocks.res_down(input=p1, dim_out=f[1], name="enc2_"+n)  #64 -> 32
    c3, p3 = blocks.res_down(input=p2, dim_out=f[2], name="enc3_"+n)  #32 -> 16
    c4, p4 = blocks.res_down(input=p3, dim_out=f[3], name="enc4_"+n)  #16->8
